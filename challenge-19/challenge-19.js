(function (){
    ...
    */
    ...
    function hasClass(markup, cssClass){
        var regexClass = new RegExp('(?:class=)(?:["\'](.+?)["\'])', 'gm');
        var stringClass = markup.match(regexClass).join('').replace(regexClass, '$1');
        var cssClassInRegex = new RegExp (cssClass);
        return cssClassInRegex.test(stringClass) + ' para a classe ' + cssClass;
    };
